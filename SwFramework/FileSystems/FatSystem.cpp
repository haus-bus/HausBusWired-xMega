...
static bool hasFreeLockEntries()
{
  ...
  {
    if ( fileSemaphors[i].isFree() ) return true;
  }
  return false;
}
...
uint32_t FatSystem::FileSystem::createChain( uint32_t clst )
{
  ...
  { /* Stretch the current chain */
    ...
    if ( cs < n_fatent ) return cs; /* It is already followed by next cluster */
    scl = clst;
  }
  ...
  if ( !(i % (fs->sectorSize / SZ_DIR)) )
  {
    ...
    else
    {
      ...
      if ( ((i / (fs->sectorSize / SZ_DIR)) & (fs->csize - 1)) == 0 )
      {
        ...
        if ( clst <= 1 ) return INT_ERR;
        if ( clst == 0xFFFFFFFF ) return DISK_ERR;
        if ( clst >= fs->n_fatent )
        {
          ...
          if ( clst == 0 ) return DENIED; /* No free cluster */
          if ( clst == 1 ) return INT_ERR;
          if ( clst == 0xFFFFFFFF ) return DISK_ERR;
          /* Clean-up stretched table */
          if ( fs->moveWindow( 0 ) ) return DISK_ERR; /* Flush active window */
          ...
          for ( c = 0; c < fs->csize; c++ )
          {
            ...
            fs->wflag = 1;
            if ( fs->moveWindow( 0 ) ) return DISK_ERR;
            fs->winsect++;
          }
...
// Create file name in directory form
  ...
  *path = &p[si]; /* Return pointer to the next segment */
  ...
  if ( !i ) return INVALID_NAME; /* Reject nul string */
...
// Load sector 0 and check if it is an FAT-VBR (in SFD)
  ...
  if ( fmt == 3 ) return DISK_ERR;
  if ( fmt ) return NO_FILESYSTEM; // No FAT volume is found
...
// An FAT volume is found. Following code initializes the file system object
  ...
  if ( !fasize ) fasize = getDwordLittle( fs->win + BPB_FATSz32 );
  fs->fsize = fasize;

  fs->n_fats = b = fs->win[BPB_NumFATs]; // Number of FAT copies
  if ( b != 1 && b != 2 ) return NO_FILESYSTEM; // (Must be 1 or 2)
  fasize *= b; // Number of sectors for FAT area

  fs->csize = b = fs->win[BPB_SecPerClus]; // Number of sectors per cluster
  if ( !b || (b & (b - 1)) ) return NO_FILESYSTEM; // (Must be power of 2)

  fs->n_rootdir = getWordLittle( fs->win + BPB_RootEntCnt ); // Number of root directory entries
  if ( fs->n_rootdir % (fs->sectorSize / SZ_DIR) ) return NO_FILESYSTEM; // (BPB_RootEntCnt must be sector aligned)

  tsect = getWordLittle( fs->win + BPB_TotSec16 ); // Number of sectors on the volume
  if ( !tsect ) tsect = getDwordLittle( fs->win + BPB_TotSec32 );

  nrsv = getWordLittle( fs->win + BPB_RsvdSecCnt ); // Number of reserved sectors
  if ( !nrsv ) return NO_FILESYSTEM; // (BPB_RsvdSecCnt must not be 0)

// Determine the FAT sub type
  sysect = nrsv + fasize + fs->n_rootdir / (fs->sectorSize / SZ_DIR); // RSV+FAT+DIR
  if ( tsect < sysect ) return NO_FILESYSTEM; // (Invalid volume size)
  nclst = (tsect - sysect) / fs->csize; // Number of clusters
  if ( !nclst ) return NO_FILESYSTEM; // (Invalid volume size)
  fmt = FileSystem::FAT12;
  if ( nclst >= MIN_FAT16 ) fmt = FileSystem::FAT16;
  if ( nclst >= MIN_FAT32 ) fmt = FileSystem::FAT32;
...
// Boundaries and Limits
  ...
  {
    if ( fs->n_rootdir ) return NO_FILESYSTEM; // (BPB_RootEntCnt must be 0)
    fs->dirbase = getDwordLittle( fs->win + BPB_RootClus ); // Root directory start cluster
    szbfat = fs->n_fatent * 4; // (Required FAT size)
  }
  else
  {
    if ( !fs->n_rootdir ) return NO_FILESYSTEM; // (BPB_RootEntCnt must not be 0)
...
{
// Load boot record
  if ( disk->read( win, sect, 1 ) != IDiskIo::OK ) return 3;

// Check record signature (always placed at offset 510 even if the sector size is >512)
  if ( getWordLittle( &win[BS_55AA] ) != 0xAA55 ) return 2;
