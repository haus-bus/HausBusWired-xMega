/*********************************************************************
 ...
 Configuration 	: debug
 Model Element	: ArrayVector
 //!	Generated Date	: Wed, 18, Jun 2014
 File Path	: SwFrameworkAvr/debug/SwFramework/Containers/ArrayVector.h
 *********************************************************************/
...
template<typename T, uint8_t MAX_SIZE> class ArrayVector
{
...
template<typename T, uint8_t MAX_SIZE> void ArrayVector<T, MAX_SIZE>::add(
    const T& val )
{
...
template<typename T, uint8_t MAX_SIZE> void ArrayVector<T, MAX_SIZE>::pushBack(
    const T& val )
{
  if ( m_size < MAX_SIZE )
  {
...
template<typename T, uint8_t MAX_SIZE> void ArrayVector<T, MAX_SIZE>::remove(
    const T& val )
{
  for ( uint8_t i = 0; i < size(); i++ )
  {
...
template<typename T, uint8_t MAX_SIZE> bool ArrayVector<T, MAX_SIZE>::hasElement(
    const T& val )
{
  for ( uint8_t i = 0; i < size(); i++ )
  {
