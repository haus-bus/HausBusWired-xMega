...
uint16_t SlotHw::getPwmDuty() {
    ...
    if( pin == Pin0 ) compareValue = timerCounter0->getCaptureA();
    else if( pin == Pin1 ) compareValue = timerCounter0->getCaptureB();   
    else if( pin == Pin2 ) compareValue = timerCounter0->getCaptureC();
    else if( pin == Pin3 ) compareValue = timerCounter0->getCaptureD();
...
uint16_t SlotHw::isOn() {
    ...
    if( isDimmerHw() )
    {
      if( pwmEnabled && timerCounter0->isRunning() ) return getPwmDuty();
...
void SlotHw::setPwmDuty(uint16_t duty) {
    ...
    if( pwmEnabled )
    {
      ...
      if( pin == Pin0 )      timerCounter0->setCompareA( compareValue );
      else if( pin == Pin1 ) timerCounter0->setCompareB( compareValue );   
      else if( pin == Pin2 ) timerCounter0->setCompareC( compareValue );
      else if( pin == Pin3 ) timerCounter0->setCompareD( compareValue );
      if( duty )
      { 
