...
void HMWModule::processEvent( uint8_t const * const frameData,
                              uint8_t frameDataLength, bool isBroadcast )
{
  ...
  {
    case '@':                                      // HBW-specifics
      if ( frameData[1] == 'a' && frameDataLength == 6 )
      {  // "a" -> address set
    ...
    case 'R':                                                     // Read EEPROM
      ...
      {                              // Length of before incoming data must be 4
        sendAck = 0;
        DEBUG_H1( FSTR( "read eeprom" ) );
        adrStart = ((unsigned int) (frameData[1]) << 8) | frameData[2]; // start adress of eeprom
        for ( uint8_t i = 0; i < frameData[3]; i++ )
        {
           nvm_read_char(INT_EEPROM, adrStart + i, &hmwrs485->txFrameData[i] );
        };
    ...
    case 'h':                            // get Module type and hardware version
      DEBUG_H1( FSTR( "Hardware Version and Type" ) );
      sendAck = 0;
    ...
    case 'v':                                            // get firmware version
      DEBUG_H1( FSTR( "Firmware Version" ) );
      sendAck = 0;
...
void HMWModule::processEventSetLevel( uint8_t channel, unsigned int level )
{
  // tell the hardware
...
void HMWModule::processEventGetLevel( uint8_t channel )
{
...
void HMWModule::processEmessage( uint8_t const * const frameData )
{
  // process E-Message
  ...
  for ( int block = 0; block <= blocknum; block++ )
  {
    ...
    {
	  uint8_t data;
	  nvm_read_char(INT_EEPROM, block * blocksize + byteIdx , &data );
      if ( data != 0xFF )
      {
        hmwrs485->txFrameData[4 + block / 8] |= (1<<(block % 8 ));
        break;
      }
    }
  };
...
void HMWModule::broadcastAnnounce( uint8_t channel )
{
...
void HMWModule::broadcastKeyEvent( uint8_t channel, uint8_t keyPressNum,
                                   uint8_t pressType )
{
  hmwrs485->txTargetAddress = 0xFFFFFFFF;  // broadcast
  hmwrs485->txFrameControlByte = 0xF8;     // control byte
...
void HMWModule::sendInfoMessage( uint8_t channel, uint8_t *ptr, uint8_t length,
...
void HMWModule::writeEEPROM( int address, uint8_t value, bool privileged )
{
  // save uppermost 4 bytes
  if ( !privileged && (address > E2END - 4) ) return;
  // write if not anyway the same value
  uint8_t data;
  nvm_read_char(INT_EEPROM, address , &data );
		
  if ( value != data ) nvm_write_char( INT_EEPROM, address, value );
}
;
...
{ 
  nvm_read(INT_EEPROM, E2END + 1 - sizeof(hmwrs485->txSenderAddress), &hmwrs485->txSenderAddress, sizeof(hmwrs485->txSenderAddress) );
...
void HMWModule::setNewId()
{
  ...
  {
    hmwrs485->txSenderAddress = SystemTime::now();
    DEBUG_H2( FSTR("Setting new ID: 0x"), hmwrs485->txSenderAddress );
    for ( uint8_t i = 0; i < 4; i++ )
      writeEEPROM( E2END - i, hmwrs485->txSenderAddress >> (i * 8), true );
  }
}

void HMWModule::determineSerial( uint8_t* buf )
{
  uint32_t tmp = hmwrs485->txSenderAddress;
  buf[0] = 'H';
  buf[1] = 'B';
  buf[2] = 'W';
