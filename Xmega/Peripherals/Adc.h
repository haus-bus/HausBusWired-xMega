...
public:
  ...
  template<uint32_t rate> inline void setClockRate()
  {
    ASSERT( rate );
    ASSERT( rate <= F_CPU );
    ...
    if ( ratio <= 4 ) reg.PRESCALER = ADC_PRESCALER_DIV4_gc;
    if ( ratio <= 8 ) reg.PRESCALER = ADC_PRESCALER_DIV8_gc;
    if ( ratio <= 16 ) reg.PRESCALER = ADC_PRESCALER_DIV16_gc;
    if ( ratio <= 32 ) reg.PRESCALER = ADC_PRESCALER_DIV32_gc;
    if ( ratio <= 64 ) reg.PRESCALER = ADC_PRESCALER_DIV64_gc;
    if ( ratio <= 256 )
  ...
  //  */
  template<bool sign, ADC_RESOLUTION_t resolution, ADC_REFSEL_t reference> inline void setConversionParameter()
  {
  ...
  //  */
  template<ADC_SWEEP_t sweep, ADC_EVSEL_t evSel, ADC_EVACT_t evAct> inline void setConversionTrigger()
  {
